    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-500">Fetching announcements board...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-850">Announcements Board</h1>
      ...
      {filteredAnnouncements.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center space-y-4">
          ...
          <div className="space-y-2 max-w-sm mx-auto">
            <h3 className="text-slate-850 font-bold">No announcements to display</h3>
        ...
        <div className="grid grid-cols-1 gap-6">
          {filteredAnnouncements.map((ann) => {
            ...
            return (
              <div
                key={ann._id}
                className="bg-white hover:bg-white border border-slate-200 backdrop-blur-md rounded-2xl p-6 shadow-lg transition duration-200"
              >
                {/* Meta details */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-slate-100 border border-slate-700 flex items-center justify-center font-bold text-indigo-400">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded-full bg-slate-100 border border-slate-700 text-[10px] font-semibold text-slate-500 uppercase tracking-widest">
                ...
                <div className="space-y-3">
                  <h3 className="text-lg font-bold text-slate-850 hover:text-brand-400 transition cursor-pointer" onClick={() => toggleExpand(ann._id)}>
                    {ann.title}
                  </h3>
                  
                  <p className={`text-sm text-slate-650 leading-relaxed whitespace-pre-wrap ${!isExpanded && 'line-clamp-3'}`}>
                    {ann.content}
                  </p>
