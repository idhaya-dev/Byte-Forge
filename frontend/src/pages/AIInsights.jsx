...
export const AIInsights = () => {
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-400">Running performance diagnostics...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 font-sans">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-black text-white">AI Academic Insights</h1>
      ...
      {/* Main Analysis Card */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-slate-900 via-violet-955/20 to-slate-900 border border-violet-900/30 p-8 shadow-xl">
        <div className="absolute top-0 right-0 w-80 h-full bg-gradient-to-l from-violet-500/10 to-transparent blur-2xl pointer-events-none"></div>
        <div className="relative z-10 space-y-4">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xxs font-bold uppercase tracking-wider text-violet-400 bg-violet-500/10 border border-violet-500/20 animate-pulse">
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
            </svg>
            Diagnostic Summary
          </div>
          <p className="text-slate-200 text-sm leading-relaxed max-w-2xl font-medium">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Card: Strengths */}
        <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl shadow-lg space-y-4 relative overflow-hidden group">
          ...
          {insights.strengths.length === 0 ? (
            ...
            <ul className="space-y-3 text-xs text-slate-350 leading-relaxed list-none">
        ...
        {insights.recommendations.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {insights.recommendations.map((rec, index) => (
              <div key={index} className="p-4 bg-slate-950/50 border border-slate-850 rounded-xl space-y-2 flex flex-col justify-between hover:border-slate-800 transition">
